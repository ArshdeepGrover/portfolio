...
export class TestimonialFormComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.seoService.updateCanonicalUrl(
      'https://studio.arshdeepgrover.dev/testimonials',
    );
    this.seoService.updateMetaTags([
      {
        name: 'description',
        content:
          'Worked with Arshdeep Studio? Share your experience and leave a testimonial.',
      },
      {
        property: 'og:title',
        content: 'Leave a Testimonial | Studio.Arshdeep',
      },
      {
        property: 'og:description',
        content: 'Share your experience working with Arshdeep Studio.',
      },
      {
        property: 'og:url',
        content: 'https://studio.arshdeepgrover.dev/testimonials',
      },
    ]);
  }
  ...
  get isValid(): boolean {
    return !!this.formData.name && !!this.formData.message && this.rating > 0;
  }
  ...
  onSubmit(): void {
    ...
    const ratingText =
      this.ratingLabels[this.rating] + ' (' + this.rating + '/5)';

    const fd = new FormData();
    fd.append('access_key', '8e889d8d-2fca-4a43-a5b8-5b5a60bd7a95');
    fd.append(
      'subject',
      'New Testimonial from ' + this.formData.name + ' - ' + ratingText,
    );
    ...
    fd.append(
      'permission',
      this.formData.permission ? 'Yes - can display publicly' : 'No',
    );
    ...
    fetch('https://api.web3forms.com/submit', { method: 'POST', body: fd })
      .then(async (response: Response) => {
        ...
        if (response.ok) {
          ...
          this.submitted = true;
          console.log(
            'Form submitted successfully, submitted state:',
            this.submitted,
          );
